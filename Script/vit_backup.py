...
#train_labels=labeled_features(:round(len(labeled_features)*70))
#test_labels=labeled_features(len(labeled_features)-(round(len(labeled_features)*70)):)
...
#test_set= [(MEMM_features(bigram,word,post_bi,previous_tag,punct)real_tag)for (bigram, word, post_bi, previous_tag, real_tag,punct) in test_labels]
#print("Train samples = "+str(len(train_labels)))
#print("Test samples = "+str(len(test_labels)))
...
def MEMM(wordList, labList):
    ...
    for w in range(4, wRange-2):
        ...
        if w>=5:
            if bookname_list[maxPreviousState]==labList[w-5]:
                total=total+1
...
#   print(len(sentenceList))
        ...
        total = total+MEMM(sentenceList,t_label_list)
        fin=fin+total
        nm=nm+len(sentenceList)
        print (total)
        print (nm)
        print ("%AGE : "+str(total/nm))
        print("after MEMM", time.strftime('%X %x %Z'))
        #print path
        print(total/nm)
    ctr=ctr+1
print("FINAL= %f" %(fin/nm))
