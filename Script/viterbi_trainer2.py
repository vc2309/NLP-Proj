...
training_file = open("../chat_history/kq.txt", "rb")
training_label_file = open("../chat_history/ks.txt", "rb")
...
# TODO
testing_file = open("../chat_history/kq.txt", "rb")
test_labels  = open("../chat_history/ks.txt","rb")
#train_ner=open("../chat_history/ner_train2.txt","rb")
#test_ner=open("../chat_history/ner_train2.txt","rb")

def MEMM_features(bigram, trigram, fgram, word, previous_tag, punct):
    features = {}
    features['bigram'] = bigram
    features['trigram'] = trigram
    features['fgram'] = fgram
...
for x, y in izip(training_file, training_label_file):
    if ctr1<=round(filelen):
        ...
        for iii in range(4, len(sentenceList)-2):
            ...
            trigram=sentenceList[iii-3]+'_'+bigram
            fgram=sentenceList[iii-4]+'_'+trigram
            item = bigram, trigram, fgram, sentenceList[iii], labelList[iii-1], labelList[iii],punct
...
print("****************************************************************")
train_set = [(MEMM_features(bigram, trigram, fgram, word, previous_tag, punct), real_tag)for (bigram, trigram, fgram, word, previous_tag, real_tag, punct) in labeled_features]
for xxx in range(len(train_set)):
    print(train_set[xxx])

print("############")
print("start training", time.strftime('%X %x %Z'))
f = open("MEMM-multi-label-vNEW1.pickle", "wb")
...
f = open('MEMM-multi-label-vNEW1.pickle', 'rb')
    ...
    for w in range(4, wRange-2):
        ...
        for t in range (tRange): # current tag
            for i in range (tRange): # prev tag
                ...
                trigram=wordList[w-3]+'_'+bigram
                fgram=wordList[w-4]+'_'+trigram
                if w == 4:
                    probability = maxent_classifier.prob_classify(MEMM_features(bigram,trigram,fgram,wordList[w],bookname_list[0],punct))
                ...
                if w > 4:
                    probability = maxent_classifier.prob_classify(MEMM_features(bigram,trigram,fgram,wordList[w],bookname_list[i],punct))
        ...
        file=open("trainer.txt",'ab')
        file2=open("errors.txt",'ab')
        print(wordList)
