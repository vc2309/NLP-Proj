...
def MEMM_features(bigram, word, post_bi, previous_tag,punct,nt):
    features = {}
...
ctr1=0
for x, y, z in izip(training_file, training_label_file, train_ner):
    if ctr1<=round(filelen*0.7):
        ...
        for iii in range(4, len(sentenceList)-2):
            ...
            nt=ner_tags[iii]
            item = bigram, sentenceList[iii], post_bi, labelList[iii-1], labelList[iii],punct,nt
...
print("****************************************************************")
train_set = [(MEMM_features(bigram, word, post_bi, previous_tag, punct,nt), real_tag)for (bigram, word, post_bi, previous_tag, real_tag,punct, nt) in labeled_features]
for xxx in range(len(train_set)):
    print(train_set[xxx])

print("############")
print("start training", time.strftime('%X %x %Z'))
f = open("MEMM-multi-label-v45.pickle", "wb")
...
f = open('MEMM-multi-label-v45.pickle', 'rb')
maxent_classifier = pickle.load(f)


def MEMM(wordList, labList, nerList):
    total=0
    start_word = "startttt"
    end_word = "endddd"
    ...
    for w in range(4, wRange-2):
        ...
        for t in range (tRange): # current tag
            for i in range (tRange): # prev tag
                ...
                if w == 4:
                    probability = maxent_classifier.prob_classify(MEMM_features(bigram,wordList[w],post_bi,bookname_list[0],punct,nt))
                ...
                if w > 4:
                    probability = maxent_classifier.prob_classify(MEMM_features(bigram,wordList[w],post_bi,bookname_list[i],punct,nt))
...
ctr=0
fin=0
for line, label, ner in zip(testing_file,test_labels,test_ner):
    if ctr>=round(0.7*filelen):
        ...
        correct=MEMM(sentenceList,t_label_list,nerList)
        total = total+correct
        fin=fin+correct
        ...
        print (" %s CORRECT OUT OF %s " %(correct,float(len(sentenceList)-6)))
