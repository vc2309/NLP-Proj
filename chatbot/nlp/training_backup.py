...
def new_feature_engineering(dialogues,intents):
	all_features = []
	feature_matrix = []
	all_slots = []
	skew_words=['what','library','book','find']
	spam=['i','it','know','what','is','please','pls','ask','tell','me','have']
	for line,intent in zip(dialogues,intents):
...
def master(path=None):
	...
	all_features, feature_matrix = new_feature_engineering(questions,intents)
	X = []
	...
	f = open("models/original/intent-logistic-classifier-new_x1.pickle", "wb")
	pickle.dump(logistic_clf , f)

	ff = open("models/original/intent-random-classifier-new_x1.pickle", "wb")
	pickle.dump(random_clf, ff)

	intent_index_file = open("models/original/intent-index-new_x1.txt", 'w+')
	intent_index_file.write('\n'.join(unique_intents))
	intent_index_file.close()

	all_feature_file = open('models/original/all_features-new_x1.txt', 'w+')
	all_feature_file.write('\n'.join(all_features))
	all_feature_file.close()

	intents_features = open('intents_features.txt','w+')
	for key in int_dict.keys():
		intents_features.write((key).upper())
		intents_features.write('\n\n')
		feats=[]
		for line in int_dict[key]:
			for item in line:
				feats.append(item)
		all_feats=list(set(feats))
		for f in all_feats:
			intents_features.write(f)
			intents_features.write('\n')
		intents_features.write('----------------')
		intents_features.write('\n')
